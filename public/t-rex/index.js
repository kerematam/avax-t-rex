...
(function () {
  ...
  Runner.prototype = {
    ...
    gameOver: function () {
      parent.postMessage(
        {
          type: "CRASHED",
          payload: Math.round(Math.ceil(this.distanceRan) * 0.025),
        },
        "/"
      );
  ...
  Horizon.prototype = {
     ...
     * @param {number} currentSpeed
     */
    addNewObstacle: async function (currentSpeed) {
      console.log("wait for sec");
      // await delay(1);
      var obstacleTypeIndex = buffer.consume(); // await getRandomNum(0, Obstacle.types.length - 1);

      console.log("obstacleTypeIndex ", obstacleTypeIndex);
  ...
  function Buffer() {
    ...
    this.feed = async (n) => {
      parent.postMessage(
        {
          type: "GAME_TO_APP",
          payload: "REQUEST_OBSTACLE_TYPE",
        },
        "/"
      );
      // await delay(1000);
      // for (let i = 0; i < n; i++) {
      //   const item = 2; //getRandomNum(0, Obstacle.types.length - 1);
      //   this.data.push(item);
      // }
    };
    this.pushData = (itemTypes) => {
      this.data.push(...itemTypes);
    };
  }

  window.addEventListener("message", function (message) {
    if (message.data.type == "FROM_PARENT") {
      console.log(message.data);
      console.log("message.data.payload ", message.data.payload);
      console.log("FROM_PARENT");
      buffer.pushData(message.data.payload);
    }
  });
})();
